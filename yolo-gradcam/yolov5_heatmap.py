...
from pytorch_grad_cam import GradCAM, XGradCAM, EigenCAM, HiResCAM, LayerCAM, RandomCAM, EigenGradCAM
...
class yolov5_target(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        result = []
        # print(int(post_result.size(0) * self.ratio))
        for i in trange(int(post_result.size(0) * self.ratio)):
            # 原始张量 post_result 中第 i 个元素从第二个位置开始到末尾的所有元素
...
class yolov5_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize):
        ...
        model = attempt_load(weight,inplace=False)
    ...
    def post_process(self, result):
        result = non_max_suppression(result, conf_thres=self.conf_threshold, iou_thres=0.2)[0]
        return result

    def draw_detections(self, box, color, name, img):
        xmin, ymin, xmax, ymax = list(map(int, list(box)))
        cv2.rectangle(img, (xmin, ymin), (xmax, ymax), tuple(int(x) for x in color), 2)
        cv2.putText(img, str(''), (xmin, ymin - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, tuple(int(x) for x in color), 2,
    ...
    def __call__(self, img_path, save_path):
        ...
        if os.path.isdir(img_path):
            ...
            self.process(img_path, f'{save_path}/{img_path}')


def get_params():
    params = {
        'weight': '../runs/train/yolov5_kmean_6402/weights/best.pt',
        'device': 'cuda:0',
        'method': 'GradCAM',
        # GradCAMPlusPlus, GradCAM, XGradCAM, EigenCAM, HiResCAM, LayerCAM, RandomCAM, EigenGradCAM
        'layer': [17, 20, 23],#25, 28, 31, 34
...
if __name__ == '__main__':
    ...
    save_path='../runs/result/YOLOv5'
    model(r'../datasets/image', save_path)