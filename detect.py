...
@torch.no_grad()
def run(weights=ROOT / 'YOLO-SOMI/weights/best.pt',  # model.pt path(s)
        source=ROOT / 'datasets/image',  # file/dir/URL/glob, 0 for webcam
        imgsz=1024,  # inference size (pixels)
        conf_thres=0.4,  # confidence threshold 0.25
    ...
    for path, img, im0s, vid_cap, s in dataset:
        ...
        if pt:
            save_dir1=''
            if visualize:
                save_dir1 = increment_path(save_dir / Path(path).stem, mkdir=True)
            out = model(img, augment=augment, visualize=visualize, save_dir=save_dir1)
            pred = out[0]
        elif onnx:
            if dnn:
                net.setInput(img)
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / 'datasets/image', help='file/dir/URL/glob, 0 for webcam')
    ...
    parser.add_argument('--name', default='yolov5', help='save results to project/name')
    ...
    opt.name= opt.name+'_'+str(opt.conf_thres)+'_'+str(opt.iou_thres)
