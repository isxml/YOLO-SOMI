...
def train(hyp,
    ...
    if not resume:
        shutil.copyfile(cfg, save_dir / 'model.yaml')
    data_dict = None
    if RANK in [-1, 0]:
    ...
    for v in model.modules():
        ...
        if isinstance(v, nn.BatchNorm2d):
            g0.append(v.weight)
        elif hasattr(v, 'weight') and isinstance(v.weight,
                                                 nn.Parameter):
            g1.append(v.weight)
    opt.adam = True
    if opt.adam:
        optimizer = Adam(g0, lr=3e-4, betas=(hyp['momentum'], 0.999))
    else:
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            ...
            if not opt.noautoanchor:
                check_anchors(dataset, model=model, thr=hyp['anchor_t'], imgsz=imgsz, save_dir=save_dir,
                              kmean=opt.kmean)
            model.half().float()
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='models/modules/yolov5_c2fcbam_odconv_bifpn_p2_seam_Decoupled.yaml',
    ...
    parser.add_argument('--cache', type=str, nargs='?', const='ram', default='',
    ...
    parser.add_argument('--name', default='yolov5_c2fcbam_bifpn_p2_test', help='save to project/name')
    ...
    parser.add_argument('--save_period', type=int, default=99, help='DDP parameter, do not modify')
    parser.add_argument('--entity', default=None, help='W&B: Entity')
    parser.add_argument('--upload_dataset', action='store_true', help='W&B: Upload dataset as artifact table')
    parser.add_argument('--bbox_interval', type=int, default=-1, help='W&B: Set bounding-box image logging interval')
    parser.add_argument('--artifact_alias', type=str, default='latest', help='W&B: Version of dataset artifact to use')
    parser.add_argument('--kmean', type=int, default=0)
